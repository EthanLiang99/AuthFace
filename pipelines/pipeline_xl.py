...
class StableDiffusionXLControlNetPipeline(
    ...
    def __init__(
        ...
        ###### we remove multi controlnet by default
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #### problem : why self.controlnet is float32 but the output is float 16?
                #### in the training code the ouput is also float 32
                down_block_res_samples = [
                    d.to(dtype=torch.float16) for d in down_block_res_samples
                ]
                ...
                # latents = latents_dict[0]

                latents_list[t] = latents
            ...
            if torch.isnan(
                ...
                def latents_to_rgb(latents):
                    ...
                    biases_tensor = torch.tensor(
                        (150, 140, 130), dtype=latents.dtype
                    ).to(latents.device)
